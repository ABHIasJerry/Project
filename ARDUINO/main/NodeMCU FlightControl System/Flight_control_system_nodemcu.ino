...
//-----------------------------------------------------------------------// 
void setup() {
pinMode(D5,OUTPUT);pinMode(D6,OUTPUT);pinMode(D7,OUTPUT);pinMode(D8,OUTPUT);pinMode(OPTOCOUPLER,OUTPUT);
delay(1300);
Serial.begin(115200);
WiFi.mode(WIFI_STA);
WiFi.begin("Abhinaba_FTTH 2.4G", "abhinaba@1993");
...
float battery_read()  // measures the battery power (3.7 Vdc) through Analog pin A0
{
    ...
    const float battery_min = 3.50;  //minimum voltage of battery before shutdown

    for (int i = 0; i < 500; i++)
    {
        sum += analogRead(Battery_sensor);
        delayMicroseconds(10);
    }
    // calculate the voltage
    voltage = sum / (float)500;
     //voltage = (voltage * 5.0) / 1023.0; //for default reference voltage
    voltage = (voltage * 3.3) / 1023.0; //for internal 1.1v reference
    //round value by two precision
    voltage = roundf((voltage * 100) / 100) * (-1);
...
void loop(){ 
digitalWrite(OPTOCOUPLER, HIGH); // sets the digital pin D0 of optocoupler to 1
...
// For TOF LiDAR sensor readings
  lox.rangingTest(&measure, false); // pass in 'true' to get debug data printout! 
  if (measure.RangeStatus != 4) {  // phase failures have incorrect data
    //height = (measure.RangeMilliMeter - 25) * 0.1; // in cm
    height = (measure.RangeMilliMeter - 25) * 0.00328084; // in ft
  } else {
    height = 0.0;
  }
...
//-----------------------------------------------------------------------//
Serial.print("Roll Angle: ");Serial.print(angle_roll_output);Serial.print("  ");
Serial.print("Pitch Angle: ");Serial.print(angle_pitch_output);Serial.print(" | ");
Serial.print("Request Roll Angle: ");Serial.print(roll_desired_angle);Serial.print("  ");
Serial.print("Request Pitch Angle: ");Serial.print(pitch_desired_angle);Serial.print(" | ");
Serial.print("Temperature: ");Serial.print(temp);Serial.println(" °C");Serial.print("  ");
Serial.print("Terrain Detected: ");Serial.print(height);Serial.println(" Ft");Serial.print("Battery Level: ");Serial.print(battery_read(), 2);Serial.print("  Flight POWER : Engaged");Serial.println();
while (gpsSerial.available() > 0)
if (gps.encode(gpsSerial.read()))
  Serial.print(" ");Serial.print("Latitude: ");Serial.print(gps.location.lat(), 6);Serial.print("  ");Serial.print("Longitude: ");Serial.print(gps.location.lng(), 6);Serial.print("  ");Serial.print("Altitude: ");Serial.print(gps.altitude.meters());Serial.print(" meters");

Serial.print(" <> ");Serial.print("Date: ");
  if (gps.date.isValid())
  {
    Serial.print(gps.date.month());Serial.print("/");Serial.print(gps.date.day());Serial.print("/");Serial.println(gps.date.year());
  }
  else
  {
...
Serial.print(" <> ");Serial.print("Time: ");
  ...
  {
    if (gps.time.hour() < 10) 
    Serial.print(F("0"));Serial.print(gps.time.hour());Serial.print(":");
    if (gps.time.minute() < 10) 
    Serial.print(F("0"));Serial.print(gps.time.minute());Serial.print(":");
    if (gps.time.second() < 10) 
    Serial.print(F("0"));Serial.print(gps.time.second());Serial.print(".");
    if (gps.time.centisecond() < 10) 
    Serial.print(F("0"));Serial.println(gps.time.centisecond());
  }
  else
  {
