...
uint8_t receiverMacAddress[] = {0xEC,0xFA,0xBC,0x5F,0x71,0x5F};  //drone mac address
struct PacketData
{
  byte lxAxisValue;
  byte lyAxisValue;
...
void OnDataSent(const uint8_t *mac_addr, esp_now_send_status_t status)
{
  ...
  Serial.println(status == ESP_NOW_SEND_SUCCESS ? "TRANSMISSION : Success" : "TRANSMISSION : Failed");
}
